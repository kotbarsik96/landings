...
class TextInputDate extends TextInputMulti {
    constructor(input) {
        ...
        this.inputs.forEach((inp, index) => {
            ...
            if (index == this.inputs.length - 1) inp.addEventListener("keydown", this.moveValueToLeft.bind(this));
...
// отдельный класс для поля телефона
class TextInputPhone extends TextInputMulti {
    constructor(node) {
        super(node);
    ...
    init() {
        this.inputWrapper.addEventListener("click", this.findClosestEmpty);
        this.inputs = Array.from(this.inputWrapper.querySelectorAll("input[type='text']"));
        this.inputs.forEach(input => {
    ...
    checkCompletion() {
        const isUncompleted = Boolean(
            this.inputs.find(inp => {
                const maxlength = inp.getAttribute("maxlength");
                return inp.value.length < maxlength ? true : false;
            })
        );
        this.isCompleted = !isUncompleted;
...
class CutImage {
    constructor(block, previewSize, isCircle = false) {
        ...
        this.img.addEventListener("pointerdown", (event) => {
            const onUp = (upEvent) => {
                if (upEvent.clientX === x && upEvent.clientY === y) {
                    setTimeout(() => this.createFullSize(true), 50);
                }
    ...
    createFullSize(doShow = false) {
        ...
        const previewCoef = this.img.width / this.previewSize;
        const ctxSize = this.origImg.width / previewCoef;
...
const forms_inputSelectors = [
    ...
    { selector: ".field__input-multi.field-init--phone", classInstance: TextInputPhone },
