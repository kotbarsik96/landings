<template>
    <div
        class="search"
        :class="{ '__active': isHideable ? isSearchShown : true }"
    >
        <div
            class="search__field"
            :class="{ '__shown-results': isResultsFoundAndShown }"
        >
            <div
                class="search__icon __icon-search"
    ...
    computed: {
        ...
        searchedProducts() {
            ...
            for (let prod of this.products) {
                ...
                if (
                    modifyString(vendorCode).indexOf(modifyString(query)) >= 0
                ) {
                ...
                for (let key in prod) {
                    if (
                        key === "name" &&
                        modifyString(prod[key]).indexOf(modifyString(query)) >=
                            0
                    ) {
                        prod.vendorCode = vendorCode;
                        searched.push(prod);
